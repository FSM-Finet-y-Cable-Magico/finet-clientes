...
| 16 | CU-32 | Solicitando cambio de contraseña de red inalámbrica | Autogestión | ✅ — `POST /portal/wifi/password` registra la solicitud en la tabla `solicitud_contrasena_wifi` (estado `PENDIENTE`, en mayúsculas por el §11.15) y la muestra en `/portal/servicios`. La clave nunca se guarda en texto plano: va **hasheada con bcrypt** (registro permanente, para verificar) y **cifrada con la llave pública RSA del CRM** (`password_nueva_cifrada`, la vía por la que CU-33 la obtiene). El portal **no** cambia la clave: la ejecución es CU-33 y corre por cuenta del CRM |
| 17 | CU-33 | Ejecutando cambio de clave WiFi solicitado por el cliente | Autogestión | ⏳ Pendiente — la ejecución real contra el equipo del cliente (ONT/router) no está implementada, corre por cuenta de G8 (su CU-40, vía Smart OLT). **Cómo obtienen la clave**: descifran `solicitud_contrasena_wifi.password_nueva_cifrada` con su llave privada RSA (nosotros ciframos con la pública, `CRM_PUBLIC_KEY`), y al marcar `APLICADA` dejan esa columna en `NULL`. El `password_nueva_hash` queda para siempre y permite verificar que lo aplicado es lo que el cliente pidió. Si la columna cifrada llega en `NULL` es que faltaba la llave: hay que pedirle la clave al cliente |
