...
| 16 | CU-32 | Solicitando cambio de contraseña de red inalámbrica | Autogestión | ✅ — `POST /portal/wifi/password` registra la solicitud en la tabla `solicitud_contrasena_wifi` (estado `pendiente`) y la muestra en `/portal/servicios`. La clave se guarda **hasheada con bcrypt**, nunca en texto plano. El portal **no** cambia la clave: la ejecución es CU-33 y corre por cuenta del CRM |
