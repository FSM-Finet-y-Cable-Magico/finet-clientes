...
| 16 | CU-32 | Solicitando cambio de contraseña de red inalámbrica | Autogestión | ✅ — `POST /portal/wifi/password` registra la solicitud en la tabla `solicitud_contrasena_wifi` (estado `PENDIENTE`, en mayúsculas por el §11.15) y la muestra en `/portal/servicios`. La clave se guarda **hasheada con bcrypt**, nunca en texto plano. El portal **no** cambia la clave: la ejecución es CU-33 y corre por cuenta del CRM |
| 17 | CU-33 | Ejecutando cambio de clave WiFi solicitado por el cliente | Autogestión | ⏳ Pendiente — la ejecución real contra el equipo del cliente (ONT/router) no está implementada. **Ojo**: `solicitud_contrasena_wifi.password_nueva_hash` es un hash bcrypt, así que la clave a aplicar no se puede leer de la tabla; CU-33 necesita definir por dónde la recibe quien la aplica (el hash solo permite verificar que lo aplicado es lo que el cliente pidió) |
